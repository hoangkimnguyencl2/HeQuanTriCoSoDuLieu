...
Go
--trigger instead of insert view Employee_EmployeeGroups_INSERT
IF OBJECT_ID('Employee_EmployeeGroups_INSERT') IS NOT NULL
DROP TRIGGER Employee_EmployeeGroups_INSERT;
GO

CREATE TRIGGER Employee_EmployeeGroups_INSERT
ON Employee_EmployeeGroups
	INSTEAD OF INSERT
AS
	DECLARE @EmployeeID int, @EmployeeName varchar(50),@GroupID int,
	@Sex Varchar(50) , @Salary money,
	@NameGroup varchar(30), @AccountID int, @TestRowCount int;
 
	SELECT @TestRowCount = COUNT(*) FROM Inserted;
	IF @TestRowCount = 1
	BEGIN
		SELECT @EmployeeID = EmployeeID, @EmployeeName = EmployeeName,
		@Sex = Sex, @Salary = Salary , @NameGroup = NameGroup ,@AccountID = AccountID
		FROM Inserted;
		IF (@NameGroup IS NOT NULL AND
		@AccountID IS NOT NULL AND @EmployeeName IS NOT NULL AND @Sex IS NOT NULL AND @Salary IS NOT NULL)
			BEGIN
			SELECT @GroupID = GroupID
			FROM EmployeeGroups
			WHERE GroupName = @NameGroup

			INSERT Employees
			(EmployeeName, Sex, Salary,
			IDGroup, AccountID)
			VALUES (@EmployeeName,@Sex, @Salary,
			@GroupID, @AccountID);
			END;
...
-- Store Lay ngay he thong
If OBJECT_ID('GetDay') is not null
